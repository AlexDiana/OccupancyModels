...
plotOccupancyIndex <- function(modelResults){
  ...
  mu_eps <- modelResults$modelOutput$mu_eps
  sd_eps <- modelResults$modelOutput$sd_eps
  
  years <- modelResults$dataCharacteristics$Years
  Y <- length(years)
  XY_centers_unique <- modelResults$dataCharacteristics$XY_centers_unique
  indexUniqueSite <- modelResults$dataCharacteristics$indexUniqueSite
  
  psi_mean_output <- matrix(NA, Y, nsims)
  for (run in 1:nsims) {
    
    print(run)
    beta_psi <- mvrnormArma(mu_beta_psi, Sigma_beta_psi)
    
    eps_unique <- simulate_eps(mu_eps, sd_eps)
    
    if(length(XY_centers_unique) > 0){
      a_s_unique <- beta_psi[Y + XY_centers_unique] + 
        eps_unique[indexUniqueSite]  
    } else {
      a_s_unique <- beta_psi[Y] + 
        eps_unique[indexUniqueSite]   
    }
    
    psi_mean_output[,run] <- as.vector(computeYearEffect(Y, a_s_unique, beta_psi))
  }
  
  CI_psi_mean <- apply(psi_mean_output, 1, function(x){
...
plotSpatialSitesEffect <- function(modelResults){
  ...
  if(usingSpatial){
    ...
    SiteEffect <- sapply(1:nSites, function(i){
      ...
      mu_beta_psi[Y + idxSite[i]] #+ eps_unique_output[i]
    }) 
...
plotBaseLineCaptureProb <- function(modelResults) {
  ...
  beta_p_output <- modelResults$modelOutput$mu_beta_p
